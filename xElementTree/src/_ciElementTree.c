/*
 * ElementTree
 * $Id: _ciElementTree.c 3473 2009-01-11 22:53:55Z fredrik $
 *
...
element_setstate_from_Python(ElementObject *self, PyObject *state)
{
    static char *kwlist[] = {PICKLED_TAG, PICKLED_ATTRIB, PICKLED_NAMES,
                             ...
                             PICKLED_CHILDREN, 0};
    PyObject *args;
    PyObject *tag, *attrib, *names, *cache, *text, *tail, *children;
    PyObject *retval;
    ...
    if (PyArg_ParseTupleAndKeywords(args, state, "|$OOOOOOOOOOO", kwlist, &tag,
                                    &attrib, &names, &cache, &text, &tail, &children))
        retval = element_setstate_from_attributes(self, tag, attrib, names,
                                                  cache, text, tail, children);
    else
        retval = NULL;
...
element_subscr(PyObject* self_, PyObject* item)
{
    ...
    if (PyIndex_Check(item)) {
        ...
        if (PySlice_GetIndicesEx((_Py_SliceObject)item,
                self->extra->length,
...
static PyTypeObject Element_Type = {
    ...
    "_ciElementTree.Element", sizeof(ElementObject), 0,
    /* methods */
...
static PyTypeObject TreeBuilder_Type = {
    PyVarObject_HEAD_INIT(NULL, 0)
    "_ciElementTree.TreeBuilder", sizeof(TreeBuilderObject), 0,
    /* methods */
...
static PyTypeObject XMLParser_Type = {
    PyVarObject_HEAD_INIT(NULL, 0)
    "_ciElementTree.XMLParser", sizeof(XMLParserObject), 0,
    /* methods */
...
#if PY_MAJOR_VERSION >= 3
static struct PyModuleDef _ielementtreemodule = {
...
MOD_INIT(_ciElementTree)
{
    ...
    #if PY_MAJOR_VERSION >= 3
        m = PyModule_Create(&_ielementtreemodule);
    #else
        m = Py_InitModule("_ciElementTree", _functions);
    #endif
    ...
    Py_INCREF(Py_True);
    PyModule_AddObject(m, "_patched_for_komodo_", Py_True);
